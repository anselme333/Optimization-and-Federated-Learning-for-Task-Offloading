...
for i in range(count_EdgeCloud):
    ...
    computation_capacity_EC.append(
        random.uniform(2500000000, 3900000000))  # Computation capacity at regional cloud in terms of Cycles Per Second
    # or Hertz  # (from 2.5 GHz to 3.9 GHz)
    communication_EC_RC.append(random.uniform(3000, 3500))  # Overall Bandwith between EC m and RC n in terms of Mbps
    computation_capacity_EC= computation_capacity_EC * count_EdgeCloud
# At RUs
...
for i in range(count_User):
    computation_capacity_user.append(
        random.randint(500000000, 1000000000))   # Computation capacity at each end-user in terms of Cycles Per Second/Hz

    computation_user_requirement.append(
        random.uniform(52500000, 93750000))   # computation requirement for end-users in terms of cycles/byte
...
for i in range(2, count_User):
    ...
    p_ec = random.choice(computation_capacity_EC)
    c_ec = random.choice(cache_capacity_EC)
...
spl1 = make_interp_spline(x_list, local_computation_cost, k=3)
local_computation_cost_smooth = spl1(xnew)


spl2 = make_interp_spline(x_list, offloading_computation_cost23, k=3)
offloading_computation_cost23_smooth = spl2(xnew)

spl3 = make_interp_spline(x_list, Offloading_computation_latency, k=3)
Offloading_computation_latency_smooth = spl3(xnew)

# Create smooth line chart
fig000, ax000 = plt.subplots(figsize=(9, 6))
local_computation, = plt.plot(xnew, local_computation_cost_smooth,  color='r', linestyle='dashed')
edge_cloud_computation, = plt.plot(xnew, offloading_computation_cost23_smooth,color='g', linestyle='solid')
regional_computation,= plt.plot(xnew, Offloading_computation_latency_smooth, color='b', linestyle='dotted')
...
spl11 = make_interp_spline(x_list, computation_capacity_allocation_user, k=3)
computation_capacity_allocation_user_smooth = spl11(xnew)


spl22 = make_interp_spline(x_list, computation_capacity_allocation_EC, k=3)
computation_capacity_allocation_EC_smooth = spl22(xnew)

spl33 = make_interp_spline(x_list, computation_capacity_allocation_dc, k=3)
computation_capacity_allocation_dc_smooth = spl33(xnew)

fig06, ax06 = plt.subplots(figsize=(9, 6))
local_computation, = plt.plot(computation_capacity_allocation_user_smooth, color='r', linestyle='dashed')# plotting by columns
edge_cloud_computation, = plt.plot(computation_capacity_allocation_EC_smooth, color='g', linestyle='solid') # plotting by columns
regional_computation, = plt.plot(computation_capacity_allocation_dc_smooth, color='b', linestyle='dotted')# plotting by columns
plt.grid(color='gray', linestyle='dashed')
plt.legend([local_computation, edge_cloud_computation, regional_computation], ['Local computation utilization',
                                                                               'EC computation utilization',
                                                                               'DC computation utilization'],
           fancybox= True,loc='upper right')
plt.xlabel('Number of iterations')
plt.ylabel('Computation resource utilization')
plt.xticks()
plt.yticks()
plt.xlim(0,80)
plt.ylim(bottom=0.)
...
def obj_dmcp1(i):
    ...
    constraint1 = [cvx.sum(percentage_radio_spectrum_vector[i]) * x <= 1,
    ...
    opt_x = x.value
    opt_g = g.value
    opt_val_varrho.append(result[1])
    print(" opt_val_varrho =   result.value",  opt_val_varrho)
    print("Type", type(opt_val_varrho))
    return opt_x,  opt_g, opt_val_varrho


#######################################################################################################################

# BCD using cyclic coordinate selection rule
t, opt_x, opt_g, opt_val_x, opt_val_g, opt_varrho, opt_val_varrho, approx_err = initialize_parameter1()
for i in range(0, max_iter):
    t = i
    opt_x,  opt_g, opt_val_cy = obj_dmcp1(t)
...
t, opt_x, opt_g, opt_val_x, opt_val_g, opt_varrho, opt_val_varrho, approx_err = initialize_parameter1()
for i in range(0, max_iter):
    i = np.random.randint(0, max_iter)
    opt_x,  opt_g, opt_val_ran = obj_dmcp1(i)
    opt_val_ran[t] = opt_val_ran[t] - 1 / 2 * np.gradient(opt_val_ran)[t]
    t += 1
    opt_val_rand = opt_val_ran
...
t, opt_x, opt_g, opt_val_x, opt_val_g, opt_varrho, opt_val_varrho, approx_err = initialize_parameter1()
for i in range(0, max_iter):
    i = np.argmax(np.abs(total_delay + varrho / 2 * user_offloading_variable))
    opt_x,  opt_g, opt_val_gou = obj_dmcp1(i)
    t += 1
    opt_val_gso = opt_val_gou


print("opt_val_cy", opt_val_cyc)
print("opt_val_rand", opt_val_rand)
...
norm1 = np.linalg.norm(opt_val_cyc)
opt_val_cyc = opt_val_cyc/norm1

norm2 = np.linalg.norm(opt_val_gso)
opt_val_gso = opt_val_gso/norm2

norm3 = np.linalg.norm(opt_val_rand)
opt_val_rand = opt_val_gso/norm3
print(opt_val_rand.min())
print(opt_val_gso.min())
print(opt_val_cyc.min())
index2 = len(opt_val_cyc)
x_list2 = list(range(0, index2))
x_list2 = np.array(x_list2)
xnew = np.linspace(x_list2.min(), x_list2.max(), index2)

spl4 = make_interp_spline(x_list2, opt_val_cyc, k=3)
opt_val_cyc = spl4(xnew)


spl44 = make_interp_spline(x_list2, opt_val_gso, k=3)
opt_val_gso = spl44(xnew)

spl444 = make_interp_spline(x_list2, opt_val_rand, k=3)
opt_val_rand = spl444(xnew)

fig2, ax2 = plt.subplots(figsize=(9, 6))
cyc, =  plt.plot(opt_val_cyc, 'r-', linewidth=2, marker='s')
gso, =  plt.plot(opt_val_gso, 'g-', linewidth=2, marker='x')
ran, =  plt.plot(opt_val_rand, 'b-', linewidth=2, marker='+')
plt.xlabel('Iterations')
plt.ylabel('Total delay minimization(Second)')
plt.legend([cyc, gso, ran], ['Our Proposal with Cyclic', 'Our Proposal with Gauss-Southwell', 'Our Proposal with Randomized'], fancybox=True,)
plt.grid(color='gray', linestyle='dashed')
plt.show()

fig3, ax3 = plt.subplots(figsize=(9, 6))
cyc3, = plt.semilogy(opt_val_cyc, 'r-', linewidth=2, marker='s')
gso3, = plt.semilogy(opt_val_gso, 'g-', linewidth=2, marker='x')
ran3, = plt.semilogy(opt_val_rand, 'b-', linewidth=2, marker='+')
plt.xlabel('Iterations')
plt.ylabel('Total delay minimization(Second)')
plt.legend([cyc3, gso3, ran3], ['Our Proposal with Cyclic', 'Our Proposal with Gauss-Southwell', 'Our Proposal with Randomized'], fancybox=True,)
plt.grid(color='gray', linestyle='dashed')
plt.show()

fig4, ax4 = plt.subplots(figsize=(9, 6))
cyc, =  plt.plot(opt_val_cyc, 'r-', linewidth=2, marker='s')
#gso, =  plt.plot(opt_val_gso, 'g-', linewidth=2, linestyle='-', marker='x')
#ran, =  plt.plot(opt_val_rand, 'b-', linewidth=2, linestyle='--', marker='+')
plt.xlabel('Iterations')
plt.ylabel('Total delay minimization(Second)')
plt.legend([cyc], ['Our proposal with Cyclic'], fancybox=True)
plt.grid(color='gray', linestyle='dashed')
plt.show()
"""
########################################################################################################################
...
def initialize_parameter2():
    t = 0  # iteration
    # Initial optimal variable at 0-th iteration  to zeros
    opt_x, opt_y,opt_varrho = np.zeros([number_demand, ]), np.zeros([number_demand, ]), np.zeros([number_demand, ])
    ...
    # Objective function
    opt_val_x.append(user_offloading_variable + (1 / (2*varrho)) *
                     np.power(np.linalg.norm(user_offloading_variable - opt_x), 2))
    opt_val_y.append(user_offloading_variable + (1 / (2*varrho)) *
                     np.power(np.linalg.norm(error_checking_variable - opt_y), 2))
    opt_val_varrho2.append(np.dot(total_delay, user_offloading_variable) + (varrho / 2) *
                          np.power(np.linalg.norm(EC_m_cache_allocation_variable - opt_varrho), 2))
    approx_err.append(np.Inf)
    return t, opt_x, opt_y, opt_val_x, opt_val_y, opt_varrho, opt_val_varrho2, approx_err


def obj_dmcp2(i):
    x = cvx.Variable(number_demand, nonneg=True)
    y = cvx.Variable(number_demand, nonneg=True)
    objective2 = cvx.Minimize(abs(cvx.sum((1 - x) * local_computation_cost[i]) +
                                  cvx.sum(cvx.multiply(x, y) * offloading_computation[i])))
    constraint2 = [cvx.sum(cvx.multiply(x, y) * computation[i]) <= np.sum(computation_capacity_EC),
                   (1 - x) + x * cvx.multiply(y, total_delay[i]) <= delay_requirement, (1 - x) + cvx.multiply(x, y) == 1,
                   y <= x, x >= 0, y >= 0, x <= 1, y <= 1]
    ...
    result2 = myprob2.solve(method='bcd')
    opt_x = x.value
    opt_y = y.value
    opt_val_varrho2.append(result2[1])
    print(" opt_val_varrho =   result.value", opt_val_varrho2)
    print("Type", type(opt_val_varrho2))
    return opt_x, opt_y,  opt_val_varrho2


########################################################################################################################

# BCD sing Cyclic coordinate selection rule
t, opt_x, opt_y, opt_val_x, opt_val_y, opt_varrho, opt_val_varrho2, approx_err = initialize_parameter2()
for i in range(0, max_iter):
    opt_x,  opt_y, opt_val_cy = obj_dmcp2(i)
    opt_val_cyc = opt_val_cy
    t += 1
########################################################################################################################
# BCD using Randomized coordinate selection rule
...
for i in range(0, max_iter):
    i = np.random.randint(0, max_iter)
    opt_x,  opt_y, opt_val_ran = obj_dmcp2(t)
    opt_val_rand = opt_val_ran
    t += 1
...
for i in range(0, max_iter):
    i = np.argmax(np.abs(total_delay + varrho / 2 * user_offloading_variable))
    opt_x,  opt_y, opt_val_gou = obj_dmcp2(t)
    opt_val_gso = opt_val_gou
    t += 1
...
fig3, ax3 = plt.subplots(figsize=(9, 6))
cyc3, = plt.semilogy(opt_val_cyc, 'r-', linewidth=2, linestyle='--', marker='s')
gso3, = plt.semilogy(opt_val_gso, 'g-', linewidth=2, linestyle='-', marker='x')
ran3, = plt.semilogy(opt_val_rand, 'b-', linewidth=2, linestyle='--', marker='+')
plt.xlabel('Iterations')
plt.ylabel('Total delay minimization (Second)')
plt.legend([cyc3, gso3, ran3], ['Our Proposal with Cyclic', 'Our Proposal with Gauss-Southwell', 'Our Proposal with Randomized'], fancybox=True,)
plt.grid(color='gray', linestyle='dashed')
