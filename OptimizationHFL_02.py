...
# We have to meet accuracy_threshold. Otherwise, we have to perform federated learning
if accuracy_threshold >= theta:
    def include(filename):
        if os.path.exists(filename):
            execfile(filename)

    include('Federated_Learning.py')
...
predicted_traffic = pd.read_csv('network_traffic_prediction.csv', engine='python')
...
for i in range(count_EdgeCloud):
    ...
    computation_capacity_EC.append(
        random.uniform(250000, 3900000))  # Computation capacity of edge cloud (EC) in Megabyte
...
for i in range(count_User):
    computation_capacity_user.append(
        random.randint(5000, 10000))   # Computation capacity at each end-user in terms of Cycles Per Second/Hz

    computation_user_requirement.append(
        random.uniform(525, 9375))   # computation requirement for end-users in terms of cycles/byte
...
local_computation, = plt.plot(xnew, sorted(local_computation_cost_smooth),  color='r', linestyle='dashed')
edge_cloud_computation, = plt.plot(xnew, sorted(offloading_computation_cost23_smooth),color='g', linestyle='solid')
regional_computation,= plt.plot(xnew, sorted(Offloading_computation_latency_smooth), color='b', linestyle='dotted')
...
for i in range(0, max_iter):
    ...
    opt_x,  opt_g, opt_val_ran = obj_dmcp1(t)
...
for i in range(0, max_iter):
    ...
    opt_x,  opt_g, opt_val_gou = obj_dmcp1(t)
...
spl4 = make_interp_spline(x_list2, opt_val_cyc, k=7)
opt_val_cyc = spl4(xnew)


spl44 = make_interp_spline(x_list2, opt_val_gso, k=7)
opt_val_gso = spl44(xnew)

spl444 = make_interp_spline(x_list2, opt_val_rand, k=7)
opt_val_rand = spl444(xnew)

fig2, ax2 = plt.subplots(figsize=(9, 6))
cyc, =  plt.plot(opt_val_cyc, 'r-',  marker='s')
gso, =  plt.plot(opt_val_gso, 'g-', marker='x')
ran, =  plt.plot(opt_val_rand, 'b-', marker='+')
...
cyc3, = plt.semilogy(opt_val_cyc, 'r-', marker='s')
gso3, = plt.semilogy(opt_val_gso, 'g-',  marker='x')
ran3, = plt.semilogy(opt_val_rand, 'b-',  marker='+')
plt.xlabel('Iterations')
plt.ylabel('Total delay minimization(Second)')
...
for i in range(0, max_iter):
    opt_x,  opt_y, opt_val_cy = obj_dmcp2(t)
    opt_val_cyc = opt_val_cy
    t += 1
########################################################################################################################
# BCD using Randomized coordinate selection rule
