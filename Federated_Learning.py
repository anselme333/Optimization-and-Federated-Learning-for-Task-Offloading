...
model = tf.keras.models.Sequential([
    tf.keras.layers.LSTM(128, return_sequences=True, input_dim=look_back),
...
model.summary()
history = model.fit(trainX, trainY, epochs=3, verbose=2, callbacks=[tensorboard_callback])
...
predicted_network_traffic = model.predict(X_test2)
...
print('Number of edge cloud datasets: {l}'.format(l=len(federated_train_data)))
...
for i in range(9):
    ...
    plt.subplot(3, 3, i+1)
    plt.title('Near_RT_RIC {}'.format(i))
...
def model_fn():
    keras_model = tf.keras.models.Sequential([tf.keras.layers.LSTM(128, return_sequences=True, input_dim=look_back),
                                               tf.keras.layers.LSTM(128, return_sequences=True),
                                               tf.keras.layers.LSTM(128, return_sequences=True),
                                               tf.keras.layers.LSTM(128, return_sequences=True),
                                               tf.keras.layers.LSTM(128), tf.keras.layers.Dense(1)])
    return tff.learning.from_keras_model(keras_model, input_spec=preprocessed_example_dataset.element_spec,
...
def keras_evaluate(state, round_num):
  ...
  loss, accuracy = tff_model.evaluate(testX, testY, steps=3, verbose=2)
...
for round_num in range(1, NUM_ROUNDS):
    ...
    federated_train_data = make_federated_data(data_fed, Near_RT_RICs)
...
# Federated Learning
state.model.assign_weights_to(tff_model)
Y_pred_tff = tff_model.predict(testX).ravel()
X_train_tff = np.ravel(testX)
Y_pred_tff = np.ravel(Y_pred_tff)
print(len(X_train_tff))
#Y_pred_tff = Y_pred_tff.reshape(-1,1)
Y_pred_tff = Y_pred_tff
print(len(Y_pred_tff))


plt.plot(X_train_tff[:240],label='Actual Network Traffic')
plt.plot(Y_pred_tff[:240], markerfacecolor='red', label="Predicted Network Traffic")
plt.legend(loc="upper left")
plt.ylabel('Normalized network traffic (Mbps)')
plt.xlabel('Time')
plt.grid(color='gray', linestyle='dashed')
plt.show()
...
federated_train_data = make_federated_data_edge_device(data_fed_edge_device, sample__edge_devices)

print('Number of edge devices: {l}'.format(l=len(federated_train_data)))
print('First dataset: {d}'.format(d=federated_train_data[0]))
...
for round_num in range(1, NUM_ROUNDS):
    ...
    federated_train_data = make_federated_data_edge_device(data_fed_edge_device, Edge_devices)
    print("ROUND ", round_num)
    keras_evaluate(state, NUM_ROUNDS + 1)
    state, metrics = iterative_process.next(state, federated_train_data)
    print('\tTrain: loss={}, accuracy={} '.format(round_num, metrics["train"]["loss"],
